    ...
    <transition name="fade-slide">
      <div v-if="visible" class="flex flex-wrap gap-2 mb-12">
        <span
          ...
          :class="[
            'cursor-pointer px-3 py-1 rounded-full text-sm font-medium transition',
...
export default defineComponent({
  ...
  props: {
    categorias: {
      type: Array as () => { id: number; categoria: string }[],
      required: true,
    },
  },
  ...
  setup(_, { emit }) {
    ...
    return { visible, toggle, selected, toggleCategoria, animating, ChevronUpIcon, ChevronDownIcon, AdjustmentsHorizontalIcon };
  },
});
</script>
